...
unsigned int nStakeTargetSpacing = 180; // 3 minutes stakes spacing
unsigned int nStakeMinAge = 60 * 60 * 24 * 1; // 1 days as zero time weight
unsigned int nStakeMaxAge = 60 * 60 * 24 * 365; // 1 year as full weight
...
unsigned int DarkGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	bnTargetLimit.SetCompact(fProofOfStake ? bnProofOfStakeLimit.GetCompact() : bnProofOfWorkLimit.GetCompact());
	int64_t nTargetSpacingInternal = fProofOfStake ? nStakeTargetSpacing : nTargetSpacing;

    if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || BlockLastSolved->nHeight < PastBlocksMin) { return bnTargetLimit.GetCompact(); }
	
	PastDifficultyAverage.SetCompact(bnTargetLimit.GetCompact());
        
	for (int i = 1; BlockReading && BlockReading->nHeight > 0 && i <= PastBlocksMax; i++)
	{
		if (fProofOfStake == BlockReading->IsProofOfStake())
		{
			CountBlocks++;
    ...
    if (nBlockTimeCount != 0 && nBlockTimeCount2 != 0)
	{
		...
		double Shift = nTargetSpacingInternal / ((SmartAverage >= 1) ? SmartAverage : 1);

		double fTargetTimespan = CountBlocks * nTargetSpacingInternal;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Infocoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Infocoin";
        CTransaction txNew;
        txNew.nTime = 1411171200;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1411171200;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4333939;

        //// debug print
	block.print();
        assert(block.hashMerkleRoot == uint256("0x79e95a2a343a277e464cf99522d1e50f91e638ff0577c24a0aae3da18c0f6bcc"));
...
unsigned char pchMessageStart[4] = { 0x49, 0x4E, 0x46, 0x4E };
